...
for teil in teil_array:
    ...
    #statement = "SELECT SNR.SNR FROM SNR WHERE SNR.SNR = '4072934236515'"
    cursor.execute(statement)
    ...
    for Ausschuss in Ausschuss_List:
        ...
        for res in result_in:
            statement = "SELECT MA.Ausprägung,R.SNR from Rückmeldung R join Objekt2Merkmalsausprägung as O2MA on R.ID = O2MA.ObjektID join Merkmalsausprägung as MA on O2MA.MerkmalsausprägungID = MA.ID WHERE O2MA.ObjektTyp = 2 AND R.SNR_ID = " + str(res[0]) + " AND MerkmalID = 39 ORDER BY Ausprägung DESC"
            #print(statement)
            cursor.execute(statement)
            ...
            if cursor.rowcount > 0:
                ...
                if k < len(result_in):
                    ...
                    if diff_time > max_time:
                        max_time = diff_time
                        #print('MAX: '+convert_from_s(max_time)+'    SNR:'+result_out[1])
    ...
    print("MIN: " +convert_from_s(min_time)+ "        MAX: " +convert_from_s(max_time)+ "        AVG: " + convert_from_s(avgZeit)+ "      Ausschussfaktor: " +str(format(avg_aus, '.2f'))+" %\n")
    i = i + 1

...
# Ausgabe
datei = open("C:/Users/picht/Desktop/Projektseminar I-490/universell-relational/Ergebnisse/002Auftrennung/AuftrennungCSV.txt","w")

datei.write("TEIL;MAX;MIN;AVG;FAILURE\n")
...
for Teil in Teil_List:
    ...
    for Ausschuss in Ausschuss_List:
        ...
        for Input in Input_List:
            ...
            statement = "SELECT MA.Ausprägung from Rückmeldung R join Objekt2Merkmalsausprägung as O2MA on R.ID = O2MA.ObjektID join Merkmalsausprägung as MA on O2MA.MerkmalsausprägungID = MA.ID WHERE O2MA.ObjektTyp = 2 AND R.SNR_ID = " + str(Input[0]) + " AND MerkmalID = 39 ORDER BY Ausprägung DESC"
            cursor.execute(statement)
    ...
    # Ausgabe
    datei.write("TEIL:"+ Teil[0]+";" +str(maxTime)+ ";" +str(minTime)+ ";" + str(avgTime)+ ";" +str(avgFail)+"\n")

...
# Ausgabe
datei = open("C:/Users/picht/Desktop/Projektseminar I-490/universell-relational/Ergebnisse/007Umrüstung/007UmrüstungCSV.txt","w")
...
for Linie in Linie_List:
    ...
    for FA in FA_List:
        ...
        statement = "SELECT Ausprägung, SNR.TEIL FROM SNR JOIN Objekt2Merkmalsausprägung O2MA ON (O2MA.ObjektID = SNR.ID AND o2MA.ObjektTyp = 1) JOIN Merkmalsausprägung MA ON MA.ID = O2MA.MerkmalsausprägungID WHERE MA.MerkmalID = 1 AND SNR.FA = '"+FA[0]+"' AND SNR.LINIE = '"+Linie[0]+"' AND SNR.SNR IS NOT NULL ORDER BY Ausprägung"
        cursor.execute(statement)
    ...
    for erg in Result_List:
        avg = erg[3]/erg[4]
        datei.write(erg[0][0]+":"+erg[0][1]+";"+str(erg[2])+";"+str(erg[1])+";"+str(avg)+"\n")

...
timeInFile = open("timesIn.txt", "w")
timeOutFile = open("timesOut.txt", "w")
...
with open('../../allin.txt', 'rt', encoding='utf-16') as csvfile:
    ...
    with open('../../allout.txt', 'rt', encoding='utf-16') as csvOutFile:     
        spamOutReader = csv.reader(csvOutFile, delimiter=';')
        rowAllOut = list(spamOutReader)
        i = 1
        for row in spamreader:
            if not row[4]=='':
                ...
                timeGesIn = 0
                timeLogIn = r.slowlog_get(r.slowlog_len())
                for time in timeLogIn:
                    if time['command'] != 'SLOWLOG RESET' and time['command'] != 'SLOWLOG LEN' and time['command'] != 'INFO':
                        timeGesIn = timeGesIn + time['duration']
                timeInFile.write(str(timeGesIn)+'\n')
                r.slowlog_reset()
                r.slowlog_reset() 
                ...
                #Zugehörigen Input-SNR-Key finden und aus Liste entfernen
                #print(i)
                rowOut = rowAllOut[i-1]
                if r.exists(rowOut[0]):
                    writeOut(i,rowOut)
                    recDate = maxDiff
                    ...
                    #Output dem Input mit geringstem Zeitabstand zuordnen
                    for inputdat in r.lrange(rowOut[0],0,-1):
                        beginDate = r.hget(inputdat,"Begin")
                        beginDatetime = datetime.strptime(beginDate, '%Y-%m-%dT%H:%M:%S.%f0')
                        endDate = rowOut[17]
                        endDatetime = datetime.strptime(endDate, '%Y-%m-%dT%H:%M:%S.%f0')
                    ...
                    if foundIn:
                        inCounterOut = conInput.partition(':')[2]
                        #listSNR = r.lpop(row[0])
                        #inCounter = listSNR.partition(':')[2]

                        dateDatetime = datetime.strptime(rowOut[17], '%Y-%m-%dT%H:%M:%S.%f0')
                        dateStamp = dateDatetime.timestamp()
                        inDateTime = datetime.strptime(r.hget(conInput,"Begin"), '%Y-%m-%dT%H:%M:%S.%f0')
                        inStamp = inDateTime.timestamp()
                        ...
                        #Input-Daten finden und Out-DS-Key in Verknuepfung schreiben
                        inList = r.lrange(inCounterOut,0,-1)
                        if len(inList) > 1:
                            con = str(inStamp)+":"+str(inCounterOut)+":"+str(inList[1])+":"+str(inSNR)
                            if float(dateStamp) > float(inList[1]):
                                newKey = (str(inStamp)+":"+str(inCounterOut)+":"+str(dateStamp)+":"+str(inSNR))
                                r.rename(con, newKey)
                                con = newKey
                                r.rpop(inCounterOut)
                                r.rpush(inCounterOut,dateStamp)
                        else:
                            con = (str(inStamp)+":"+str(inCounterOut)+":"+str(inStamp)+":"+str(inSNR))
                            newKey = (str(inStamp)+":"+str(inCounterOut)+":"+str(dateStamp)+":"+str(inSNR))
                            #print(con)
                            r.rename(con,newKey)
                            r.rpush(inCounterOut,dateStamp)
                        ...
                        r.lset("con",int(inCounterOut)-1,con)

...
class OnMyWatch:
    ...
    watchDirectory = "C:/Users/liepe/Desktop/Projektseminar/htw/data_simulation/target"
...
class Handler(FileSystemEventHandler):
    ...
    def on_any_event(event):
        if event.is_directory:
            ...
            if "in_data" in event.src_path:
                df = pd.read_csv(event.src_path, sep = ';', names = ["DATE", "FA", "NR", "TEIL", "SNR", "LINIE", "E","ScanE","MessageE","A2","V2","A1","V1","UseM3","UseM1","UseM2","Delta","Fehler","Span","ChargeM1","ChargeM2","ChargeM3","ScanA","MessungA","LagerIn","LagerOut","Begin"], index_col=None)
            ...
            if "out_data" in event.src_path:
                snr = None
                return_df = []
                obj_id = None

                df = pd.read_csv(event.src_path, sep = ';', names = ["SNR", "LINIE","TRÄGER", "MAT", "NAHT", "DistMmX", "DistMmy", "AngleGrad", "LengthMM", "LengthDiffMM", "AngleDiffGrad", "AxisDistMMX", "AxisDistMMY", "AxisDistMM", "RTotalNominal", "RTotalCurrent", "RCount", "Date"], index_col=None)
                df.reset_index(drop = True, inplace = True)
                df['SNR'] = df['SNR'].astype(str)
                df['LINIE'] = df['LINIE'].astype(str)
